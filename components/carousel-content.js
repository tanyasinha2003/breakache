      ...
      {data.map((item, index) => (
        <div 
          ...
          className={`carousel-content-container flex mt-[4rem] lg:mt-[4.5rem] space-x-5 lg:space-x-10 ${
            index % 2 === 1 ? 'flex-row-reverse  lg:flex-row lg:justify-end' : ''
          }`}
        >
          <Carousel images={item.images} delay={3000 + index * 200} />
          
          <div className={`content flex flex-col  w-[13rem] lg:w-[22rem]
          ${
            index % 2 === 1 ? 'items-end pr-[1.25rem] lg:pr-0 lg:items-start' : 'items-start'
          }
          `}>
            <h3 className={`text-[1.5rem] lg:text-[3.1rem] font-livvic text-left
            
            `}>
              <span className="text-red">{item.heading} </span>
              <span className="text-dark-gray">Breaks</span>
            </h3>
            <p className= {`lg:text-[1.25rem] text-[0.8rem] w-[8rem] text-dark-gray lg:w-[16rem] 
             ${
